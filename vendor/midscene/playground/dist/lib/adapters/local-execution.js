...
class LocalExecutionAdapter extends external_base_js_namespaceObject.BasePlaygroundAdapter {
    ...
    async runConnectivityTest(aiConfig) {
        const modelConfigManager = new env_namespaceObject.ModelConfigManager(aiConfig);
        return (0, core_namespaceObject.runConnectivityTest)({
    ...
    async executeAction(actionType, value, options) {
        ...
        try {
            ...
            try {
                result = await (0, external_common_js_namespaceObject.executeAction)(agent, actionType, actionSpace, value, options);
            } catch (error) {
    ...
    async getInterfaceInfo() {
        ...
        try {
            ...
            const size = 'function' == typeof this.agent.interface.size ? await this.agent.interface.size() : void 0;
            const actionTypes = 'function' == typeof this.agent.interface.actionSpace ? this.agent.interface.actionSpace().map((action)=>action?.name).filter((name)=>'string' == typeof name) : void 0;
            return {
                type,
                description,
                ...size ? {
    ...
    async getRuntimeInfo() {
        ...
        try {
            return (0, external_runtime_metadata_js_namespaceObject.buildRuntimeInfo)({
                ...
                interfaceDescription: this.agent.interface.describe?.() || void 0,
                supportsScreenshot: 'function' == typeof this.agent.interface.screenshotBase64,
