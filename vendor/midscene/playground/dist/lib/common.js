...
const agentPromptAPISet = new Set(agentPromptAPIs);
function getAgentPromptAPI(activeAgent, actionType) {
    if (!agentPromptAPISet.has(actionType)) return;
    const methodName = actionType;
    const method = activeAgent[methodName];
    if ('function' != typeof method) return;
    return async (prompt, options)=>method.call(activeAgent, prompt || '', options);
}
...
async function executeAction(activeAgent, actionType, actionSpace, value, options) {
    const canForwardDeepThink = 'aiAct' === actionType || 'runMarkdown' === actionType;
    if (!canForwardDeepThink && void 0 !== options.deepThink) console.warn('[Playground] Received deepThink in non-aiAct action options. deepThink is expected to be used with aiAct/runMarkdown during migration.', {
        actionType,
    ...
    if (action && 'function' == typeof activeAgent.callActionInActionSpace) if (value.params) {
        ...
        const { deviceOptions: _, reportDisplay: __, ...optionsWithoutDeviceOptions } = options;
        const actionParams = {
            locate: detailedLocateParam,
            ...optionsWithoutDeviceOptions,
            ...options.deviceOptions || {}
        };
        return await activeAgent.callActionInActionSpace(action.name, actionParams);
    }
    {
        const prompt = value.prompt;
        if ('aiAssert' === actionType) {
            const { reportDisplay: _reportDisplay, ...optionsWithoutReportDisplay } = options;
