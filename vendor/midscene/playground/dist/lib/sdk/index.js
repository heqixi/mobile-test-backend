...
class PlaygroundSDK {
    createAdapter(type, serverUrl, agent, agentFactory) {
        switch(type){
            case 'local-execution':
                if (!agent && !agentFactory) throw new Error('Agent or agentFactory is required for local execution');
                return new local_execution_js_namespaceObject.LocalExecutionAdapter(agent, agentFactory);
    ...
    runtimeMetadataAdapter() {
        if (this.adapter instanceof local_execution_js_namespaceObject.LocalExecutionAdapter || this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter;
        return null;
    }
    ...
    async runConnectivityTest(aiConfig) {
    ...
    onDumpUpdate(callback) {
        ...
        else if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) this.adapter.onDumpUpdate(callback);
    }
    onProgressUpdate(callback) {
        if (this.adapter instanceof local_execution_js_namespaceObject.LocalExecutionAdapter) this.adapter.setProgressCallback(callback);
    }
    async cancelExecution(requestId) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) {
        ...
        } else if (this.adapter instanceof local_execution_js_namespaceObject.LocalExecutionAdapter) {
            const result = await this.adapter.cancelTask(requestId);
            if (result.success) return {
                dump: result.dump || null,
                reportHTML: result.reportHTML || null,
    ...
    async interact(payload) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.interact(payload);
        return {
            ok: false,
            error: 'Direct interaction requires remote execution'
        };
    }
    async startRecorderSession(sessionId) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.startRecorderSession(sessionId);
        return {
            ok: false,
            supported: false,
    ...
    async getRecorderCapabilities() {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.getRecorderCapabilities();
        return {
            supported: false,
            source: 'unsupported',
            error: 'Recorder requires remote execution'
    ...
    async stopRecorderSession() {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.stopRecorderSession();
        return {
            ok: true
        };
    }
    async getRecorderEvents(since = 0) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.getRecorderEvents(since);
        return {
            events: [],
            nextIndex: since
        };
    ...
    async describeRecorderEventAtPoint(event) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.describeRecorderEventAtPoint(event);
        return {
            ok: false,
    ...
    async getRecorderScreenshotAsset(assetId) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.getRecorderScreenshotAsset(assetId);
        return null;
    }
    getRecorderScreenshotAssetUrl(assetId) {
        return this.adapter.getRecorderScreenshotAssetUrl(assetId);
    }
    async clearRecorderScreenshotAssets(sessionId) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) await this.adapter.clearRecorderScreenshotAssets(sessionId);
    }
    async pruneRecorderScreenshotAssets(sessionId, assetIds) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) await this.adapter.pruneRecorderScreenshotAssets(sessionId, assetIds);
    }
    ...
    async getSessionInfo() {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.getSessionInfo();
        return null;
    }
    async getSessionSetup(input) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.getSessionSetup(input);
        return null;
    }
    async listSessionTargets() {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.listSessionTargets();
        return [];
    }
    async createSession(input) {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.createSession(input);
        throw new Error('Session creation is only supported in server mode');
    }
    async destroySession() {
        if (this.adapter instanceof remote_execution_js_namespaceObject.RemoteExecutionAdapter) return this.adapter.destroySession();
    ...
    constructor(config){
        ...
        _define_property(this, "beforeActionHook", void 0);
