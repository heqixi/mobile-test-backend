...
import { basename, dirname, join, resolve as external_node_path_resolve, sep } from "node:path";
import { fileURLToPath } from "node:url";
import { describeElementAtPoint, runConnectivityTest } from "@midscene/core";
import { getModelRuntime } from "@midscene/core/ai-model";
import { getTmpDir, sleep } from "@midscene/core/utils";
import { getMidsceneRunSubDir } from "@midscene/shared/common";
import { PLAYGROUND_SERVER_PORT } from "@midscene/shared/constants";
import { ModelConfigManager, globalModelConfigManager, overrideAIConfig } from "@midscene/shared/env";
...
import { antiEscapeScriptTag, uuid as utils_uuid } from "@midscene/shared/utils";
...
function createElementDescriberRuntime(agent) {
    ...
    return {
        ...
        describeModelRuntime: modelConfigManager ? getModelRuntime(modelConfigManager.getModelConfig('insight')) : missingModelRuntime,
        locateModelRuntime: modelConfigManager ? getModelRuntime(modelConfigManager.getModelConfig('default')) : missingModelRuntime
    };
}
...
function getRecorderScreenshotAssetRoot() {
    const root = external_node_path_resolve(getMidsceneRunSubDir('output'), RECORDER_SCREENSHOT_ASSET_DIR);
    mkdirSync(root, {
        recursive: true
    });
    return root;
...
function recorderScreenshotAssetPath(assetId, mimeType) {
    ...
    return assertPathInsideDirectory(root, join(root, `${safeAssetId}.${recorderScreenshotAssetExtension(mimeType)}`));
}
...
function findRecorderScreenshotAssetPath(assetId) {
    ...
    for (const extension of [
        ...
        const filePath = assertPathInsideDirectory(root, join(root, `${safeAssetId}.${extension}`));
...
function removeRecorderScreenshotAssetsForSession(sessionId) {
    ...
    for (const entry of readdirSync(root, {
        withFileTypes: true
    }))if (entry.isFile() && entry.name.startsWith(prefix)) rmSync(join(root, entry.name), {
        force: true
    });
...
const debugScreenshot = getDebug('playground:screenshot', {
    console: true
});
const debugMjpeg = getDebug('playground:mjpeg', {
    console: true
});
const debugInteract = getDebug('playground:interact', {
    console: true
});
const debugCancel = getDebug('playground:cancel', {
    console: true
});
const debugReport = getDebug('playground:report', {
    console: true
});
...
class PlaygroundServer {
    ...
    async enrichStudioPreviewRecorderEventWithAiDescribe(event, agent) {
        ...
        try {
            ...
            const describeResult = await withTimeout(describeElementAtPoint(elementDescriber, [
                x,
                y
            ], {
                verifyPrompt,
    ...
    filePathForUuid(uuid) {
        ...
        const resolvedPath = external_node_path_resolve(filePath);
        const resolvedTmpDir = external_node_path_resolve(this.tmpDir);
    ...
    async registerReportFile(reportPath) {
        ...
        const hasExternalScreenshots = await stat(screenshotsPath).then((entry)=>entry.isDirectory()).catch(()=>false);
        const format = 'index.html' === basename(reportPath) && hasExternalScreenshots ? 'html-and-external-assets' : 'single-html';
        const now = Date.now();
    ...
    setupRoutes() {
        ...
        this._app.get('/reports/:reportId/screenshots/:assetName', async (req, res)=>{
            ...
            const externalAssetPath = join(dirname(report.path), 'screenshots', req.params.assetName);
            const externalAssetExists = await stat(externalAssetPath).then((entry)=>entry.isFile()).catch(()=>false);
...
export { InteractParamsValidationError, PlaygroundServer, buildInteractParams, buildPlaygroundBrowserUrl, createManualExecutorContext, server as default, resolvePlaygroundBrowserHost, resolvePlaygroundListenHost, serializeZodField };
