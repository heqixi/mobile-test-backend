...
const debug = (0, logger_namespaceObject.getDebug)('web:bridge:agent-cli-side');
...
const getBridgePageInCliSide = (options)=>{
    ...
    const fileChooserBridgeMethods = new Set([
        external_common_js_namespaceObject.BridgeEvent.RegisterFileChooserAccept,
        external_common_js_namespaceObject.BridgeEvent.ClearFileChooserAccept,
        external_common_js_namespaceObject.BridgeEvent.GetFileChooserError
    ]);
    const syncFileChooserError = async ()=>{
        if (!fileChooserEnabled) return;
        try {
            const error = await server.call(external_common_js_namespaceObject.BridgeEvent.GetFileChooserError, [], 5000);
        ...
        get (target, prop, receiver) {
            ...
            if ('registerFileChooserListener' === prop) return async (handler)=>{
                ...
                try {
                    await handler({
                        accept: async (files)=>{
                            await server.call(external_common_js_namespaceObject.BridgeEvent.RegisterFileChooserAccept, [
                                files
                            ]);
                        }
                    });
                ...
                return {
                    dispose: ()=>{
                        fileChooserEnabled = false;
                        server.call(external_common_js_namespaceObject.BridgeEvent.ClearFileChooserAccept, [], 5000).catch((error)=>{
