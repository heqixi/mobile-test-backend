...
__webpack_require__.d(__webpack_exports__, {
    ...
    buildDownloadBehavior: ()=>buildDownloadBehavior,
...
function buildDownloadBehavior(downloadPath) {
    ...
    return {
        policy: 'allow',
        downloadPath: external_node_path_default().resolve(downloadPath)
    };
}
...
async function launchPuppeteerPage(target, preference, browser, existingPage) {
    ...
    if (target.extraHTTPHeaders) {
        const normalizedHeaders = Object.fromEntries(Object.entries(target.extraHTTPHeaders).map(([key, value])=>[
                key,
                String(value)
            ]));
...
exports.buildDownloadBehavior = __webpack_exports__.buildDownloadBehavior;
