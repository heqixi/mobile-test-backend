...
import type { LocateResultAdapter, LocateResultCoordinates, ResolvedLocateResultCoordinates } from '../shared/model-locate-result/types';
...
interface CustomPlanningLifecycle<TParsed = unknown> {
    ...
    transformActions(parsed: TParsed, input: CustomPlanningInput): PlanningAction[];
...
export interface CustomPlanningDefinition<TParsed = unknown> extends CustomPlanningLifecycle<TParsed> {
    coordinates: LocateResultCoordinates;
}
export interface ResolvedCustomPlanningDefinition<TParsed = unknown> extends CustomPlanningLifecycle<TParsed> {
    coordinateSystem: ResolvedLocateResultCoordinates;
...
export interface CustomPlanningMessageConfig<TParsed = unknown> {
    ...
    buildAssistantContent?: (parsed: TParsed, rawResponse: string, input: CustomPlanningInput) => string | undefined;
}
export {};
