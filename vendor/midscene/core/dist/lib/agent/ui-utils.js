...
function paramStr(task) {
    ...
    if ('Planning' === task.type) if ('Locate' === task.subType) value = locateParamStr(task?.param);
        ...
        const planTask = task;
        value = planTask.output?.log || planTask.param?.userInstructionDisplay || (planTask.param?.userInstruction ? (0, external_common_js_namespaceObject.userPromptToString)(planTask.param.userInstruction) : void 0);
    }
