...
class TaskBuilder {
    ...
    async handleActionPlan(plan, context) {
        ...
        const action = (0, external_common_js_namespaceObject.findActionInActionSpaceOrThrow)(planType, actionSpace);
        const param = plan.param;
        const locateFields = (0, index_js_namespaceObject.findAllMidsceneLocatorField)(action.paramSchema);
        const requiredLocateFields = (0, index_js_namespaceObject.findAllMidsceneLocatorField)(action.paramSchema, true);
        locateFields.forEach((field)=>{
            if (param[field]) {
                const locatePlan = locatePlanForLocate(param[field]);
                debug('will prepend locate param for field', `action.type=${planType}`, `param=${JSON.stringify(param[field])}`, `locatePlan=${JSON.stringify(locatePlan)}`, `hasLocatedPixelBbox=${(0, external_utils_js_namespaceObject_1.ifPlanLocateParamHasLocatedPixelBbox)(param[field])}`);
    ...
    createLocateTask(plan, detailedLocateParam, context, onResult) {
        ...
        const taskLocator = {
            ...
            executor: async (param, taskContext)=>{
                ...
                const paramWithLocatedPixelBbox = (0, external_utils_js_namespaceObject_1.ifPlanLocateParamHasLocatedPixelBbox)(param) ? param : void 0;
                (0, utils_namespaceObject.assert)(param?.prompt || paramWithLocatedPixelBbox, `No prompt or id or position or locatedPixelBbox to locate, param=${JSON.stringify(param)}`);
                ...
                const applyDump = (dump)=>{
                    ...
                    task.usage = (0, external_usage_intent_js_namespaceObject.withUsageIntent)(dump.taskInfo?.usage, 'default');
                    task.searchArea = dump.taskInfo?.searchArea;
                    if (dump.taskInfo?.searchAreaUsage) task.searchAreaUsage = (0, external_usage_intent_js_namespaceObject.withUsageIntent)(dump.taskInfo.searchAreaUsage, 'default');
                    if (dump.taskInfo?.reasoning_content) task.reasoning_content = dump.taskInfo.reasoning_content;
                };
                const planLocatedElement = paramWithLocatedPixelBbox ? (0, external_utils_js_namespaceObject_1.matchElementFromPlan)(paramWithLocatedPixelBbox) : void 0;
                ...
                const elementFromCacheResult = isPlanDirectHit || isXpathHit ? null : await (0, external_utils_js_namespaceObject_1.matchElementFromCache)({
                ...
                if (element) {
                    ...
                    if (invalidElementReason) {
                        if (locateDump) throw new external_types_js_namespaceObject.ServiceError(invalidElementReason, locateDump);
