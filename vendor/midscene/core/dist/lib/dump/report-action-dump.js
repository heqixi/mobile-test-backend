...
function replacerForDumpSerialization(_key, value) {
    if ('screenshotSequence' === _key) return;
...
function reviverForDumpDeserialization(key, value) {
    ...
    external_screenshot_item_js_namespaceObject.ScreenshotItem.isSerialized(value);
    return value;
}
class ExecutionDump {
    serialize(indents) {
    ...
    collectScreenshots() {
        ...
        for (const task of this.tasks){
            if (task.uiContext?.screenshot instanceof external_screenshot_item_js_namespaceObject.ScreenshotItem) screenshots.push(task.uiContext.screenshot);
            if (task.recorder) {
                for (const record of task.recorder)if (record.screenshot instanceof external_screenshot_item_js_namespaceObject.ScreenshotItem) screenshots.push(record.screenshot);
            }
        }
        return screenshots;
    }
...
class ReportActionDump {
    ...
    serializeWithInlineScreenshots(indents) {
        const processValue = (obj)=>{
            if (obj instanceof external_screenshot_item_js_namespaceObject.ScreenshotItem) return {
                base64: obj.base64,
                capturedAt: obj.capturedAt
            };
            if (Array.isArray(obj)) return obj.map(processValue);
            if (obj && 'object' == typeof obj) {
                const entries = Object.entries(obj).filter(([key])=>'screenshotSequence' !== key).map(([key, value])=>[
                        key,
                        processValue(value)
                    ]);
    ...
    serializeToFiles(basePath) {
        ...
        for (const screenshot of screenshots){
            const imagePath = (0, external_node_path_namespaceObject.join)(screenshotsDir, `${screenshot.id}.${screenshot.extension}`);
            if ((0, external_node_fs_namespaceObject.existsSync)(imagePath)) continue;
            const rawBase64 = screenshot.rawBase64;
            (0, external_node_fs_namespaceObject.writeFileSync)(imagePath, Buffer.from(rawBase64, 'base64'));
        }
        (0, external_node_fs_namespaceObject.writeFileSync)(basePath, this.serialize(), 'utf-8');
    }
    static fromFilesAsInlineJson(basePath) {
        const dumpString = (0, external_node_fs_namespaceObject.readFileSync)(basePath, 'utf-8');
        const screenshotsDir = `${basePath}.screenshots`;
        const loadFromExecutionScreenshotDir = (id, mimeType)=>{
            const ext = 'image/jpeg' === mimeType ? 'jpeg' : 'png';
            const filePath = (0, external_node_path_namespaceObject.join)(screenshotsDir, `${id}.${ext}`);
            if (!(0, external_node_fs_namespaceObject.existsSync)(filePath)) return '';
            const data = (0, external_node_fs_namespaceObject.readFileSync)(filePath);
            return `data:image/${ext};base64,${data.toString('base64')}`;
        };
        const dumpData = JSON.parse(dumpString);
        const store = new external_screenshot_store_js_namespaceObject.ScreenshotStore({
            mode: 'directory',
            reportPath: basePath
        });
        const processedData = (0, external_screenshot_restoration_js_namespaceObject.restoreImageReferences)(dumpData, (ref)=>{
...
//# sourceMappingURL=report-action-dump.js.map