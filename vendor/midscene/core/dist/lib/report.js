...
const screenshotModeAttrRegExp = new RegExp(`${html_utils_js_namespaceObject.DATA_SCREENSHOT_MODE_ATTR}="(inline|directory)"`);
function readDeclaredScreenshotMode(reportFilePath) {
    let mode;
    try {
        (0, html_utils_js_namespaceObject.streamDumpScriptsSync)(reportFilePath, ({ openTag })=>{
...
function isDirectoryModeReport(reportFilePath) {
    if ('index.html' !== external_node_path_namespaceObject.basename(reportFilePath)) return false;
    const declared = readDeclaredScreenshotMode(reportFilePath);
    if (declared) return 'directory' === declared;
    return (0, external_node_fs_namespaceObject.existsSync)(external_node_path_namespaceObject.join(external_node_path_namespaceObject.dirname(reportFilePath), 'screenshots'));
}
function isDirectoryBasedReport(reportFilePath) {
    return 'index.html' === external_node_path_namespaceObject.basename(reportFilePath);
}
...
function tryParseAgentReportDump(dumpString) {
    ...
    try {
        return external_types_js_namespaceObject.ReportActionDump.fromSerializedString(trimmed);
    } catch  {
        return null;
    }
}
function mergedAgentReportComment(reports) {
    ...
    if (1 === reports.length) return (0, html_utils_js_namespaceObject.generateAgentReportComment)(reports[0]);
    ...
    const mergedReport = new external_types_js_namespaceObject.ReportActionDump({
        sdkVersion: reports[0].sdkVersion || (0, external_utils_js_namespaceObject.getVersion)(),
    ...
    return (0, html_utils_js_namespaceObject.generateAgentReportComment)(mergedReport);
}
class ReportMergingTool {
    ...
    mergeReports(reportFileName = 'AUTO', opts) {
        ...
        const targetDir = outputDir ? external_node_path_namespaceObject.resolve(outputDir) : (0, common_namespaceObject.getMidsceneRunSubDir)('report');
        if (outputDir) (0, external_node_fs_namespaceObject.mkdirSync)(targetDir, {
            recursive: true
        });
        ...
        try {
            ...
            for(let i = 0; i < this.reportInfos.length; i++){
                ...
                if (reportInfo.reportFilePath) {
                    if (isDirModeByIndex[i]) {
                        ...
                        if ((0, external_node_fs_namespaceObject.existsSync)(screenshotsDir)) {
                ...
                const reportHtmlStr = `${(0, external_utils_js_namespaceObject.reportHTMLContent)({
                    ...
                    attributes: {
                        ...
                        [html_utils_js_namespaceObject.DATA_SCREENSHOT_MODE_ATTR]: hasDirectoryModeReport ? 'directory' : 'inline',
            ...
            if (agentComment) (0, external_node_fs_namespaceObject.appendFileSync)(outputFilePath, agentComment);
