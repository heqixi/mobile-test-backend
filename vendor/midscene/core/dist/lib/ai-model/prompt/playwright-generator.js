...
const generatePlaywrightTest = async (events, options, model)=>{
    const modelRuntime = resolveModelRuntime(model);
    (0, external_recorder_generation_common_js_namespaceObject.validateEvents)(events);
    const summary = (0, external_recorder_generation_common_js_namespaceObject.prepareEventSummary)(events, {
    ...
    const screenshots = (0, external_recorder_generation_common_js_namespaceObject.getScreenshotsForLLM)(events, options.maxScreenshots || 3);
...
Important: Return ONLY the raw Playwright test code. Do NOT wrap the response in markdown code blocks (no \`\`\`typescript, \`\`\`javascript or \`\`\`). Start directly with the code content.`;
    const messageContent = (0, external_recorder_generation_common_js_namespaceObject.createMessageContent)(promptText, screenshots, false !== options.includeScreenshots);
...
const generatePlaywrightTestStream = async (events, options, model)=>{
    const modelRuntime = resolveModelRuntime(model);
    (0, external_recorder_generation_common_js_namespaceObject.validateEvents)(events);
    const summary = (0, external_recorder_generation_common_js_namespaceObject.prepareEventSummary)(events, {
        testName: options.testName,
