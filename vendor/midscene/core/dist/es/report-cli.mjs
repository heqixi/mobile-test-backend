...
import { z } from "zod";
import { extractAllDumpScriptsSync } from "./dump/html-utils.mjs";
import { resolveScreenshotSource } from "./dump/screenshot-store.mjs";
import { deriveCaseStatus } from "./dump/task-status.mjs";
...
function deriveReportAttributesFromHtml(htmlPath, index) {
    ...
    try {
        const { baseDump, executions } = collectDedupedExecutions(htmlPath);
        const testStatus = readSourceTestStatus(htmlPath) ?? deriveCaseStatus(executions);
        return {
            testId: fallbackId,
