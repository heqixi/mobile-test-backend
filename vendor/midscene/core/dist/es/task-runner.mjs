...
class TaskRunner {
    ...
    async append(task, options) {
        ...
        const appended = [];
        if (Array.isArray(task)) appended.push(...task.map((item)=>this.markTaskAsPending(item)));
        else appended.push(this.markTaskAsPending(task));
        this.tasks.push(...appended);
        if ('running' !== this.status) this.status = 'pending';
        await this.emitSnapshotChange();
        for (const appendedTask of appended)await this.emitTaskEvent('append', appendedTask);
    }
    ...
    async flush(options) {
        ...
        if (cancelledTasks.length > 0) {
            await this.emitSnapshotChange();
            for (const cancelledTask of cancelledTasks)await this.emitTaskEvent('cancel', cancelledTask);
        }
        let finalizeError;
    ...
    constructor(name, uiContextBuilder, options){
        ...
        _define_property(this, "onSnapshotChange", void 0);
        _define_property(this, "onTaskEvent", void 0);
