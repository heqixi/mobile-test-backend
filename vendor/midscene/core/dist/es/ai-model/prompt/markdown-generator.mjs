...
function normalizeGeneratedMarkdown(content) {
    ...
    const fencedMatch = trimmed.match(/^```(?:md|markdown)?\s*([\s\S]*?)\s*```$/i);
...
async function convertRecordLogIntoMarkdown(log, modelConfig) {
    return generateRecorderMarkdownReplay(log, modelConfig);
}
export { convertRecordLogIntoMarkdown, createRecorderMarkdownReplayPrompt, generateRecorderMarkdownReplay };

//# sourceMappingURL=markdown-generator.mjs.map