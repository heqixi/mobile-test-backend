...
const warnObserver = getDebug('ui-observer', {
    console: true
});
...
class UIObserver {
    ...
    async stop() {
        ...
        try {
            ...
            if (this.source && this.frames.length > 0) {
                ...
                if (lastDecoded) {
                    representative.screenshot = ScreenshotItem.create(lastDecoded, lastFrame.capturedAt);
    ...
    async buildObservedUIContext() {
        assert(this.stopped && this.representative, 'call observer.stop() before asserting on the observed window');
        ...
        if (uncachedRefs.length > 0) {
            ...
            assert(results.length === uncachedRefs.length, 'frame source decode() must return one image per frame handle');
        ...
        const sequence = sampled.map((frame)=>ScreenshotItem.create(this.decodedCache.get(frame.ref), frame.capturedAt));
...
//# sourceMappingURL=ui-observer.mjs.map